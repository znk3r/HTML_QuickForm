...
class HTML_QuickForm_RuleRegistry
{
    ...
    public function registerRule($ruleName, $type, $data1, $data2 = null)
    {
        ...
        } elseif ('function' == $type || 'callback' == $type) {
            // Callback function
    ...
    public function validate($ruleName, $values, $options = null, $multiple = false)
    {
        ...
        if (is_array($values) && !$multiple) {
            ...
            foreach ($values as $value) {
                if (true === $rule->validate($value, $options)) {
                    ++$result;
                }
            }
    ...
    public function _getJsValue($element, $elementName, $reset = false, $index = null)
    {
        ...
        if (is_a($element, 'html_quickform_group')) {
            ...
            for ($i = 0, $count = count($elements); $i < $count; ++$i) {
                $append = ('select' == $elements[$i]->getType() && $elements[$i]->getMultiple()) ? '[]' : '';
        ...
        } elseif ('checkbox' == $element->getType()) {

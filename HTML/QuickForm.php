 ...
 * @see HTML_QuickForm::registerElementType(), HTML_QuickForm::getRegisteredTypes(), HTML_QuickForm::isTypeRegistered()
 ...
 * @see HTML_QuickForm::registerRule(), HTML_QuickForm::getRegisteredRules(), HTML_QuickForm::isRuleRegistered()
...
class HTML_QuickForm extends HTML_Common
{
    ...
    /**
     * Datasource object implementing the informal datasource protocol.
     *
     * @var object
     */
    public $_datasource;
    ...
    public function setDefaults($defaultValues = null, $filter = null)
    {
        if (is_array($defaultValues)) {
            ...
            $this->_defaultValues = static::arrayMerge($this->_defaultValues, $defaultValues);
    ...
    public function setConstants($constantValues = null, $filter = null)
    {
        if (is_array($constantValues)) {
            ...
            $this->_constantValues = static::arrayMerge($this->_constantValues, $constantValues);
            foreach (array_keys($this->_elements) as $key) {
                $this->_elements[$key]->onQuickFormEvent('updateValue', null, $this);
            }
        }
    ...
    public function createElement($elementType)
    {
        ...
        return $this->_loadElement('createElement', $elementType, array_slice($args, 1));
    }
    ...
    public function _loadElement($event, $type, $args)
    {
        ...
        if (!static::isTypeRegistered($type)) {
        ...
        if (isset($this->_submitValues[$elementName]) || isset($this->_submitFiles[$elementName])) {
            ...
            if (is_array($value) && isset($this->_submitFiles[$elementName])) {
                foreach ($this->_submitFiles[$elementName] as $k => $v) {
                    $value = static::arrayMerge($value, $this->_reindexFiles($this->_submitFiles[$elementName][$k], $k));
                }
            }
            ...
            if ((is_array($value) || null === $value) && isset($this->_submitFiles[$base])) {
                ...
                if (null !== $fileValue) {
                    $value = null === $value ? $fileValue : static::arrayMerge($value, $fileValue);
                }
            }
        }
     ...
     *
     * @param mixed $valueSome value from the $_FILES array
    ...
    public function addGroupRule($group, $arg1, $type = '', $format = null, $howmany = 0, $validation = 'server', $reset = false)
    {
        ...
        if (is_array($arg1)) {
            ...
            if (is_a($groupObj, 'html_quickform_group')) {
                ...
                if ('required' == $type && 'radio' == $groupObj->getGroupType()) {
                    $howmany = (0 == $howmany) ? 1 : $howmany;
                } else {
                    $howmany = (0 == $howmany) ? count($groupObj->getElements()) : $howmany;
                }
            }
     ...
     *
     * @param mixed $rule Callback, either function name or array($object, 'method')
     *
     * @throws HTML_QuickForm_Error
     */
    ...
    {
        if (0 == count($this->_rules) && 0 == count($this->_formRules) &&
            $this->isSubmitted()) {
        ...
        foreach ($this->_rules as $target => $rules) {
            ...
            foreach ($rules as $rule) {
                if ((isset($rule['group'], $this->_errors[$rule['group']])) ||
    ...
    public function process($callback, $mergeFiles = true)
    {
        ...
        $values = (true === $mergeFiles) ? static::arrayMerge($this->_submitValues, $this->_submitFiles) : $this->_submitValues;
    ...
    {
        return $mergeFiles ? static::arrayMerge($this->_submitValues, $this->_submitFiles) : $this->_submitValues;
    }
     ...
     *
     * @param bool $collectHidden Whether to collect hidden elements (passed to the Renderer's constructor)
     *
     ...
     *
     * @param null|mixed $elementList Array/string of element names, whose values we want. If not set then return all elements.
     *
     * @throws HTML_QuickForm_Error
     *
        ...
        if (null === $elementList) {
            ...
            foreach (array_keys($this->_elements) as $key) {
                ...
                if (is_array($value)) {
                    ...
                    $values = static::arrayMerge($values, $value);
                }
            }
        } else {
            if (!is_array($elementList)) {
